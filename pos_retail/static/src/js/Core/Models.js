...
odoo.define('pos_retail.model', function (require) {
    ...
    models.load_models([{
        label: 'Your Odoo Server IP/Port and All POS Boxes', model: 'pos.iot', condition: function (self) {
            if (self.config.posbox_save_orders && self.config.posbox_save_orders_iot_ids.length) {
                return true
            } else {
                return false;
            }
        }, fields: [], domain: function (self) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        async _bind_iot() {
            ...
            for (let i = 0; i < this.iot_boxes_save_orders.length; i++) {
                ...
                await iot.rpc('/pos/ping/server', {
                    ip: this.config.posbox_save_orders_server_ip, port: this.config.posbox_save_orders_server_port
                }, {shadow: true, timeout: 2500}).then(function (result) {
                    let value = JSON.parse(result);
                    let response_ping_odoo_server = value.values;
                    if (!response_ping_odoo_server) {
                        self.set('synch', {
                            status: 'disconnected', pending: 'Disconnected: ' + iot.origin
                        });
                        self.alert_message({
                            title: _t('ERROR !!!'),
                    ...
                    self.set('synch', {
                        status: 'disconnected', pending: 'Connection Lose: ' + iot.origin
                    });
                    self.alert_message({
                        title: _t('ERROR !!!'), body: _t('Could not connecting POSBOX IP Address: ' + iot.origin)
                    })
                });
        ...
        async show_purchased_histories(client) {
            ...
            if (!client) {
                this.alert_message({
                    title: 'Warning', body: 'We could not find purchased orders histories, please set client first'
                });
                ...
                if (orders.length) {
                    const {confirmed, payload: result} = await this.showTempScreen('PosOrderScreen', {
                        order: null, selectedClient: client
                    });
                } else {
                    this.alert_message({
                        title: 'Warning',
        ...
        async _getVoucherNumber() {
            return await rpc.query({
                model: 'pos.config', method: 'get_voucher_number', args: [[], this.config.id], context: {}
            });
        },
        ...
        async _validate_action(body) {
            ...
            if (!validate) {
                this.alert_message({
                    ...
                    body: this.env._t('Your Action required Approve by Your Manager'),
                });
                return false;
            }
            return true
        ...
        async _validate_by_manager(body) {
            ...
            if (manager_validate.length > 1) {
                let {confirmed, payload: selected_user} = await Gui.showPopup('SelectionPopup', {
                    title: popup_title, list: manager_validate,
                })
                if (confirmed) {
        ...
        _search_read_by_model_and_id: function (model, ids) {
            ...
            return new Promise(function (resolve, reject) {
                rpc.query({
                    model: model, method: 'search_read', domain: [['id', 'in', ids]], fields: object.fields
                }, {
        ...
        initialize: function (session, attributes) {
            ...
            wait_currency.fields.push('rate');
            ...
            posCategoryModel.fields = posCategoryModel.fields.concat(['is_category_combo', 'sale_limit_time', 'from_time', 'to_time', 'submit_all_pos', 'pos_branch_ids', 'pos_config_ids', 'category_type', 'image_128',]);

            let productCategoryModel = this.get_model('product.category');
            productCategoryModel.fields = productCategoryModel.fields.concat(['complete_name', 'parent_id', 'child_id', 'write_date'])
            ...
            let productFields = ['name', 'is_credit', 'multi_category', 'multi_uom', 'multi_variant', 'supplier_barcode', 'is_combo', 'sale_ok', 'combo_limit', 'uom_po_id', 'barcode_ids', 'pos_categ_ids', 'supplier_taxes_id', 'volume', 'weight', 'description_sale', 'description_picking', 'type', 'cross_selling', 'standard_price', 'product_margin', 'pos_sequence', 'is_voucher', 'sale_with_package', 'pizza_modifier', 'qty_warning_out_stock', 'write_date', 'is_voucher', 'combo_price', 'is_combo_item', 'name_second', 'note_ids', 'tag_ids', 'commission_rate', 'company_id', 'uom_ids', 'attribute_line_ids', 'product_template_attribute_value_ids', 'addon_id', 'college_id', 'model_id', 'sex_id', 'product_brand_id', 'discountable', 'refundable', 'open_price', 'plu_number', 'minimum_price',]
            ...
            let partnerFields = ['display_name', 'ref', 'vat', 'comment', 'discount_id', 'credit', 'debit', 'balance', 'limit_debit', 'wallet', 'property_product_pricelist', 'property_payment_term_id', 'is_company', 'write_date', 'birthday_date', 'group_ids', 'title', 'company_id', 'pos_loyalty_point', 'pos_loyalty_type', 'pos_order_count', 'pos_total_amount', 'type', 'parent_id', 'company_type', 'active',]
            ...
            paymentMethodObject.fields = paymentMethodObject.fields.concat(['journal_id', 'fullfill_amount', 'shortcut_keyboard', 'cheque_bank_information', 'apply_charges', 'fees_amount', 'fees_type', 'fees_product_id', 'discount', 'discount_type', 'discount_amount', 'discount_product_id',]);
            ...
            if (res_users_object) {
                res_users_object.fields = res_users_object.fields.concat(['pos_security_pin', 'barcode', 'pos_config_id', 'partner_id', 'company_ids',]);
            ...
            if (employee_model) {
                ...
                employee_model.fields = employee_model.fields.concat(['allow_discount', 'allow_qty', 'allow_price', 'allow_remove_line', 'allow_minus', 'allow_payment', 'allow_customer', 'allow_add_order', 'allow_remove_order', 'allow_add_product', 'allow_payment_zero', 'image_1920', 'user_id',])
        ...
        async _required_set_client() {
            ...
            if (!client && this.config.add_customer_before_products_already_in_shopping_cart) {
                try {
                    const {confirmed, payload: newClient} = await Gui.showTempScreen('ClientListScreen', {
        ...
        async add_new_order() {
            const newOrder = _super_PosModel.add_new_order.apply(this, arguments);
            if (newOrder) {
                const client = newOrder.get_client()
                if (!client && this.config.customer_default_id) {
                    ...
                    if (!client_default) {
                        this.alert_message({
                            ...
                            body: this.config.customer_default_id[1] + this.env._t(' set to Default Customer of new Order, but it Archived. Please Unarchive')
                        })
                    } else {
                        newOrder.set_client(client_default);
                    }
                }
                this._required_set_client()
                if (this.products_default_new_cart) {
                    for (let i = 0; i < this.products_default_new_cart.length; i++) {
                        let p_default = this.products_default_new_cart[i]
                        let product_id = p_default.product_id[0]
                        let product = this.db.get_product_by_id(product_id)
                        if (!product) continue
                        await newOrder.add_product(product, {quantity: p_default.quantity, merge: true});
                    }
                }
            }
        ...
        format_date: function (date) { // covert datetime backend to pos
            if (date) {
                return this.formatDateTime(moment(date), {}, {timezone: true});
            } else {
                return ''
            }
        },
        ...
        // TODO: only scan ean13 calling this function
        async scan_product(parsed_code) {
            ...
            if (product) {
                let packsOfProduct = this.packaging_by_product_id[product.id]
                if (packsOfProduct.length) {
                    if (packsOfProduct.length > 1) {
                        let list = packsOfProduct.map(pack => ({
                            label: pack.name + this.env._t(' with barcode ' + pack.barcode), item: pack, id: pack.id
                        }))
                        let {confirmed, payload: packSelected} = await Gui.showPopup('SelectionPopup', {
                            title: _t('Select one Product Packaging'), list: list,
                        })
                        if (confirmed) {
                            let productOfPack = this.db.product_by_id[packSelected.product_id[0]];
                            if (productOfPack) {
                                await selectedOrder.add_product(productOfPack, {quantity: 1, merge: true});
                                let order_line = selectedOrder.get_selected_orderline();
                                order_line.price_manually_set = true;
                                let priceOfPack = packSelected['list_price']
                                if (priceOfPack > 0) {
                                    order_line.set_unit_price(packSelected['list_price']);
                                }
                                if (packSelected.pack_uom_id && packSelected.pack_uom_id[0] == productOfPack.uom_id[0] && priceOfPack > 0) {
                                    order_line.set_unit(packSelected.pack_uom_id[0], priceOfPack)
                                }
                                return true
                            }
                        }
                    } else {
                        let packSelected = packsOfProduct[0]
                        let productOfPack = this.db.product_by_id[packSelected.product_id[0]]
                        if (productOfPack) {
                            await selectedOrder.add_product(productOfPack, {quantity: 1, merge: true})
                            let order_line = selectedOrder.get_selected_orderline()
                            ...
                            if (packSelected.pack_uom_id && packSelected.pack_uom_id[0] != productOfPack.uom_id[0]) {
                                order_line.set_unit(packSelected.pack_uom_id[0], priceOfPack)
                            }
                            return true
                        }
            ...
            if (products_by_supplier_barcode) {
                let list = products_by_supplier_barcode.map(p => ({
                    id: p.id, label: p.display_name, item: p

                }))
                if (product) {
                    list.push({
                        id: product.id, label: product.display_name, item: product
                    })
                }
                let {confirmed, payload: productSelected} = await Gui.showPopup('SelectionPopup', {
                    title: _t('Select one product'), list: list,
                });
                if (confirmed) {
                    if (parsed_code.type === 'price') {
                        await selectedOrder.add_product(productSelected, {
                            quantity: 1, price: product['lst_price'], merge: true
                        });
                    } else if (parsed_code.type === 'weight') {
                        await selectedOrder.add_product(productSelected, {
                            quantity: 1, price: product['lst_price'], merge: false
                        });
            ...
            if (!product && barcodes) { // not have product but have barcodes
                ...
                if (barcodes.length > 1) {
                    let list = barcodes.map(b => ({
                        id: b.id, item: b, label: b.product_id[1] + this.env._t(' with Units: ') + b.uom_id[1]
                    }));
                    let {confirmed, payload: barcodeSelected} = await Gui.showPopup('SelectionPopup', {
                        title: _t('Select Product add to Cart'), list: list,
                    })
                    if (confirmed) {
            ...
            if (!product && barcodeScanned) {
                try {
                    let voucher = await this.rpc({
                        model: 'pos.voucher', method: 'get_voucher_by_code', args: [barcodeScanned],
                    })
                    if (voucher != -1) {
            ...
            if (orderReturn) {
                const {confirmed, payload: result} = await Gui.showTempScreen('PosOrderScreen', {
                    order: orderReturn, selectedClient: null
                });
                return true
            }
            return false
        ...
        _get_voucher_env: function (voucher) {
            ...
            return {
                widget: this, pos: this, cashier: cashier, company: company, voucher: voucher
            };
        },
        ...
        _render_vouchers: function (vouchers_created) {
            ...
            for (let i = 0; i < vouchers_created.length; i++) {
                ...
                el_pos_receipt.append(qweb.render('VoucherCard', this._get_voucher_env(voucher)));
            }
        },
        ...
        _save_to_server: function (orders, options) {
            ...
            return _super_PosModel._save_to_server.call(this, orders, options).then(function (backendOrderValue) {
                ...
                if (reason['code'] == -32098) {
                    self.alert_message({
                        title: _t('Error'), body: _t('Your Odoo Offline Mode, could not save Order to backend')
                    })
                }
        ...
        async downloadInvoiceInBackground(server_ids) {
            const {confirmed, payload: result} = await Gui.showPopup('ConfirmPopup', {
                title: _t('Invoice created successfully'), body: _t('Are you want download it')
            })
            if (confirmed) {
                const [orderWithInvoice] = await this.rpc({
                    method: 'read', model: 'pos.order', args: [server_ids, ['account_move']], kwargs: {load: false},
                });
        ...
        add_return_order: function (order_return, lines) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                let line = new models.Orderline({}, {
                    pos: this, order: order, product: product,
                });
                ...
                if (pack_operation_lots) {
                    ...
                    for (let i = 0; i < pack_operation_lots.length; i++) {
                        ...
                        if (pack_operation_lot.lot_id) {
                            multi_lot_ids.push({
                                'id': pack_operation_lot.lot_id[0], 'quantity': pack_operation_lot.quantity
                            })
                        } else {
                ...
                if (line_return.discount) line.set_discount(line_return.discount);
                if (line_return.discount_reason) line.discount_reason = line_return.discount_reason;
        ...
        add_refill_order: function (order, lines) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                let line = new models.Orderline({}, {
                    pos: this, order: newOrder, product: product,
                });
                ...
                line.set_unit_price(price);
                line.price_manually_set = true;
                if (line_refill.discount) line.set_discount(line_refill.discount);
                if (line_refill.discount_reason) line.discount_reason = line_refill.discount_reason;
        ...
        load_server_data_by_model: function (model) {
            ...
            const loaded = new Promise(function (resolve, reject) {
                let params = {
                    model: model.model, context: _.extend(context, session.user_context || {}),
                };
                if (model.ids) {
                    params.method = 'read';
                    ...
                    try {    // catching exceptions in model.loaded(...)
                        Promise.resolve(model.loaded(self, result, tmp)).then(function () {
                            ...
                            if (err.message && err.message.code == -32098) {
                                self.alert_message({
                                    title: _t('Error'), body: _t('Your Odoo or Internet is Offline')
                                })
                            }
                            resolve(err)
                        });
    ...
    models.Product = models.Product.extend({
        ...
        get_price: function (pricelist, quantity, price_extra, uom_id) {
            ...
            for (let i = 0; i < pricelist.items.length; i++) {
                ...
                if ((!item.product_tmpl_id || item.product_tmpl_id[0] === self.product_tmpl_id) && (!item.product_id || item.product_id[0] === self.id) && (!item.categ_id || _.contains(category_ids, item.categ_id[0])) && (!item.pos_category_id || _.contains(pos_category_ids, item.pos_category_id[0])) && (!item.date_start || moment(item.date_start).isSameOrBefore(date)) && (!item.date_end || moment(item.date_end).isSameOrAfter(date))) {
        ...
        get_pricelist_item_applied: function (pricelist, quantity, uom_id) {
            ...
            for (let i = 0; i < pricelist.items.length; i++) {
                let item = pricelist.items[i];
                if ((!item.product_tmpl_id || item.product_tmpl_id[0] === this.product_tmpl_id) && (!item.product_id || item.product_id[0] === this.id) && (!item.categ_id || _.contains(category_ids, item.categ_id[0])) && (!item.pos_category_id || _.contains(pos_category_ids, item.pos_category_id[0])) && (!item.date_start || moment(item.date_start).isSameOrBefore(date)) && (!item.date_end || moment(item.date_end).isSameOrAfter(date))) {
                    pricelist_items.push(item)
                }
            }
    ...
    models.Paymentline = models.Paymentline.extend({
        ...
        set_amount: function (value) {
            ...
            if (this.payment_method && this.payment_method.pos_method_type == 'rounding') {
                return this.pos.alert_message({
                    title: _t('Warning'), body: _t('Rounding Method not allow modifiers. Please set new Payment Method')
                })
            }
