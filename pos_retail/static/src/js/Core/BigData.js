odoo.define('pos_retail.big_data', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: async function (session, attributes) {
            ...
            if (this.product_model && this.partner_model) {
                ...
                this.rpc({
                    model: 'pos.cache.database', method: 'save_parameter_models_load', args: [[], models]
                })
            }
            ...
            pos_session_object.domain = function (self) {
                ...
                var domain = [['state', 'in', ['opening_control', 'opened']],];
        ...
        async getDatasByModel(model, domain, fields, context) {
            ...
            this.set_synch('connecting', _t('Sync ' + model))
            try {
                let datas = await this.rpc({
                    model: model, method: 'search_read', domain: domain, fields: fields, context: context,
                }, {
                    shadow: true,
                })
        ...
        async reloadPosScreen() {
            ...
            return new Promise(function (resolve, reject) {
                self.rpc({
                    model: 'pos.session', method: 'update_required_reinstall_cache', args: [[self.pos_session.id]]
                }, {
                    shadow: true, timeout: 65000
                }).then(function (state) {
        ...
        _save_to_server: function (orders, options) {
            if (orders.length && !options['draft']) {
                ...
                if (this.config.display_onhand && this.product_onhand_by_location_id[location.id]) for (let i = 0; i < orders.length; i++) {
        ...
        async _loadProductsFromOffsetDomain(domain, offset) {
            ...
            ProductIds = await this.rpc({
                model: 'product.product', method: 'search', args: [domain], kwargs: {
                    offset: 10000 * offset, limit: 10000, order: 'write_date'
        ...
        async _syncProducts() {
            ...
            {
                // TODO: if activeIds.length == 0, stop loop because have not any record write_date bigger than this.db.write_date_by_model['res.partner']]
                console.warn('>>> stop _syncProducts: ' + activeIds.length)
            }
        ...
        async _loadPartnersFromOffsetDomain(domain, offset) {
            ...
            PartnerIds = await this.rpc({
                model: 'res.partner', method: 'search', args: [domain], kwargs: {
                    offset: 10000 * offset, limit: 10000, order: 'write_date'
                }, context: this.env.session.user_context,
            }, {
                shadow: true,
        ...
        async _syncPartners() {
            ...
            {
                // TODO: if activeIds.length == 0, stop loop because have not any record write_date bigger than this.db.write_date_by_model['res.partner']]
                console.warn('>>> stop _syncPartners: ' + activeIds.length)
            }
        ...
        async syncProductsPartners(shadow = false) {
            console.log('=========== BEGIN UPDATE PRODUCTS AND CUSTOMERS =============')
            const partnerLastWriteDate = this.db.write_date_by_model['res.partner']
            const productLastWriteDate = this.db.write_date_by_model['product.product']
            console.warn('>> Last time updated of res.partner: ' + partnerLastWriteDate)
            console.warn('>> Last time updated of product.product: ' + productLastWriteDate)
            if (!shadow) {
                this._syncProducts()
                this._syncPartners()
            } else {
            ...
            console.log('=========== END UPDATE PRODUCTS AND CUSTOMERS =============')
            return {
                partnerLastWriteDate: partnerLastWriteDate, productLastWriteDate: productLastWriteDate
            }
        },

        autoSyncProductsPartners() {
            this._syncProducts()
            this._syncPartners()
            if (this.config.cache_sync_automatic && this.config.cache_sync_automatic_time > 0) {
                setTimeout(_.bind(this.autoSyncProductsPartners, this), this.config.cache_sync_automatic_time * 1000)
            }
        },
        ...
        api_install_datas: function (model_name) {
            ...
            let installed = new Promise(function (resolve, reject) {
                function installing_data(model_name, min_id, max_id) {
                    ...
                    self.rpc({
                        model: 'pos.cache.database', method: 'install_data', args: [null, model_name, min_id, max_id]
        ...
        saveQueryLog(key, result) {
            ...
            try {
                rpc.query({
                    model: 'pos.query.log', method: 'updateQueryLogs', args: [[], {
        ...
        async after_load_server_data() {
            ...
            if (this.cache_type == 'browse') {
                this.syncProductsPartners()
                if (this.config.cache_sync_automatic && this.config.cache_sync_automatic_time > 0) {
                    this.autoSyncProductsPartners()
                }
            }
            if (this.stock_location_ids && this.stock_location_ids.length > 0 && this.config.display_onhand) {
                this.getStockDatasByLocationIds([], this.stock_location_ids)
            }
            return res
        },
                    ...
                    if (index >= self.models.length) {
                        ...
                        if (model.model) {
                            ...
                            let requestString = JSON.stringify({
                                modelCall, fields, domain, context, ids, order
                            });
                            var params = {
                                model: model.model, context: _.extend(context, self.session.user_context || {}),
                            };
        ...
        set_last_write_date_by_model: function (model, results) {
            ...
            try {
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (!record.write_date) continue
                    if (!this.write_date_by_model[model]) {
                        this.write_date_by_model[model] = record.write_date;
                        continue;
                    }
                    if (this.write_date_by_model[model] != record.write_date && new Date(this.write_date_by_model[model]).getTime() < (new Date(record.write_date).getTime() + 1000)) {
                        this.write_date_by_model[model] = record.write_date
                    }
                }
                ...
                // --------------------********------------------------
                console.warn('>>> Last Write Date of model ' + model + ' is ' + this.write_date_by_model[model])
            } catch (e) {
                console.error(e)
            }
    ...
    models.load_models([{
        ...
        label: 'Products & Customers', installed: true, condition: function (self) {
    ...
    models.load_models([{
        label: 'Products', condition: function (self) {
        ...
        label: 'Customers', condition: function (self) {
        ...
        fields: ['create_date', 'name', 'date_order', 'user_id', 'amount_tax', 'amount_total', 'amount_paid', 'amount_return', 'to_ship', 'pricelist_id', 'partner_id', 'sequence_number', 'session_id', 'state', 'account_move', 'picking_ids', 'picking_type_id', 'location_id', 'note', 'nb_print', 'pos_reference', 'payment_journal_id', 'fiscal_position_id', 'ean13', 'expire_date', 'is_return', 'is_returned', 'voucher_id', 'email', 'write_date', 'config_id', 'is_paid_full', 'partial_payment', 'session_id', 'shipping_id', 'served_by_id',],
        domain: function (self) {
            let domain = [];
            ...
            domain.push(['state', 'not in', ['draft', 'cancel']])
            return domain
        },
        ...
        fields: ['name', 'notice', 'product_id', 'price_unit', 'qty', 'price_subtotal', 'price_subtotal_incl', 'discount', 'order_id', 'plus_point', 'redeem_point', 'promotion', 'promotion_reason', 'is_return', 'uom_id', 'user_id', 'note', 'discount_reason', 'create_uid', 'write_date', 'create_date', 'config_id', 'variant_ids', 'returned_qty', 'pack_lot_ids',],
        domain: function (self) {
        ...
        label: 'POS Payment', model: 'pos.payment', condition: function (self) {
            return false
        }, fields: ['payment_date', 'pos_order_id', 'amount', 'payment_method_id', 'name',], domain: function (self) {
            return [['pos_order_id', 'in', self.order_ids]]
        }, loaded: function (self, payments) {
            self.savePosPaymentLines(payments)
        }
    }, {
        label: 'POS Pack Operation Lot', model: 'pos.pack.operation.lot', condition: function (self) {
            return false
        }, fields: ['lot_name', 'pos_order_line_id', 'product_id', 'lot_id', 'quantity',], domain: function (self) {
        ...
        label: 'Sale Orders',
        model: 'sale.order',
        fields: ['create_date', 'pos_config_id', 'pos_location_id', 'name', 'origin', 'client_order_ref', 'state', 'date_order', 'validity_date', 'user_id', 'partner_id', 'pricelist_id', 'invoice_ids', 'partner_shipping_id', 'payment_term_id', 'note', 'amount_tax', 'amount_total', 'picking_ids', 'delivery_address', 'delivery_date', 'delivery_phone', 'book_order', 'payment_partial_amount', 'payment_partial_method_id', 'write_date', 'ean13', 'pos_order_id', 'write_date', 'reserve_order', 'reserve_from', 'reserve_to', 'reserve_table_id', 'reserve_no_of_guests', 'reserve_mobile', 'ean13', 'pos_order_id',],
        domain: function (self) {
            let domain = [];
            return domain
        },
        ...
        fields: ['name', 'discount', 'product_id', 'order_id', 'price_unit', 'price_subtotal', 'price_tax', 'price_total', 'product_uom', 'product_uom_qty', 'qty_delivered', 'qty_invoiced', 'tax_id', 'variant_ids', 'state', 'write_date'],
        condition: function (self) {
            return false
        },
        domain: function (self) {
        ...
        model: 'account.move',
        fields: ['create_date', 'name', 'date', 'ref', 'state', 'move_type', 'auto_post', 'journal_id', 'partner_id', 'amount_tax', 'amount_total', 'amount_untaxed', 'amount_residual', 'invoice_user_id', 'payment_reference', 'payment_state', 'invoice_date', 'invoice_date_due', 'invoice_payment_term_id', 'stock_move_id', 'write_date', 'currency_id',],
        condition: function (self) {
            return false
        },
        domain: function (self) {
        ...
        fields: ['move_id', 'move_name', 'date', 'ref', 'journal_id', 'account_id', 'sequence', 'name', 'quantity', 'price_unit', 'discount', 'debit', 'credit', 'balance', 'price_subtotal', 'price_total', 'write_date'],
        condition: function (self) {
            return false
        },
        domain: function (self) {
        ...
        set_client: function (client) {
            if (!this.pos.the_first_load && client && client['id'] && this.pos.deleted['res.partner'] && this.pos.deleted['res.partner'].indexOf(client['id']) != -1) {
                ...
                return this.env.pos.alert_message({
                    title: this.env._t('Warning'), body: this.env._t('This client deleted from backend')
                })
            }
