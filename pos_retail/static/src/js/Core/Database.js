...
odoo.define('pos_retail.database', function (require) {
    ...
    _super_db.add_products = function (products) {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            // todo 1. save some new variables
            ...
            // todo 4. save variants each product
            ...
            if (product.available_in_pos) {
                ...
                // todo 5 (product id is unique of stored_categories[categ_id])
                ...
                for (let j = 0, jlen = ancestors.length; j < jlen; j++) {
                    ...
                    // todo 6 (product id is unique of stored_categories[ancestor])
        ...
        this.set_last_write_date_by_model('product.product', products)
        console.log('Restored Products: ' + this.product_ids.length)
    }

    _super_db.add_partners = function (partners) {
        if (partners.length == 0) {
            console.warn('Partners not found')
            return 0
        }
        let updated_count = 0
        let new_write_date = ''
        ...
        for (let i = 0, len = partners.length; i < len; i++) {
            partner = partners[i];
            if (!partner.active || (partner.company_id && partner.company_id[0] != window.posmodel.company.id)) {
                console.warn('not allow stored partner because active is false or not the same company. partner ID: ' + partner.id)
                continue
            }
            ...
            if (partner.pos_loyalty_point && window.posmodel && window.posmodel.currency && window.posmodel.currency.rounding) {
                partner.pos_loyalty_point = round_pr(partner.pos_loyalty_point, window.posmodel.currency.rounding);
            }
            if (partner.balance && window.posmodel && window.posmodel.currency && window.posmodel.currency.rounding) {
            ...
            partner.address = (partner.street ? partner.street + ', ' : '') + (partner.zip ? partner.zip + ', ' : '') + (partner.city ? partner.city + ', ' : '') + (partner.state_id ? partner.state_id[1] + ', ' : '') + (partner.country_id ? partner.country_id[1] : '');
            let partner_string = this._partner_search_string(partner)
            this.partner_string_by_id[partner.id] = partner_string
            this.partner_search_string += this.partner_string_by_id[partner.id]
        }
        ...
        this.set_last_write_date_by_model('res.partner', partners)
        console.log('Restored Partners: ' + this.partner_ids.length)
    ...
    _super_db.search_product_in_category = function (category_id, query) {
        ...
        for (var i = 0; i < this.limit; i++) {
            ...
            if (r) {
                ...
                if (!(product.active && product.available_in_pos) || product_ids.includes(id)) continue;
    ...
    _super_db.search_partner = function (query) {
        ...
        for (var i = 0; i < this.limit; i++) {
            ...
            if (r) {
                var id = Number(r[1]);
                if (partner_ids.includes(id)) continue;
    ...
    db.include({
        init: function (options) {
            ...
            this.card_search_string_by_id = {};
        },

        get_ids_to_remove_from_server: function () {
            let unpaid_orders_to_remove = this._super();
            console.log(unpaid_orders_to_remove)
            unpaid_orders_to_remove = unpaid_orders_to_remove.filter(o => o != false)
            console.log(unpaid_orders_to_remove)
            return unpaid_orders_to_remove
        },

        _force_update_partners(partners) {
            const self = this;
            partners.forEach(p => {
                self.partner_by_id[p.id] = p
                if (p.barcode) {
                    self.partner_by_barcode[p.barcode] = p
                }
                p.address = (p.street ? p.street + ', ' : '') + (p.zip ? p.zip + ', ' : '') + (p.city ? p.city + ', ' : '') + (p.state_id ? p.state_id[1] + ', ' : '') + (p.country_id ? p.country_id[1] : '');
                let partner_string = self._partner_search_string(p)
                self.partner_search_string += partner_string;
                self.partner_string_by_id[p.id] = partner_string;
            })
        },
        ...
        get_partner_string: function (partner) {
            ...
            for (let product_id in this.product_string_by_id) {
                ...
                source.push({
                    value: product_id, label: label
                })
            }
            return source;
        },

        _card_search_string: function (card) {
            let str = card.code;
            str += '|' + card.name;
            if (card.partner_id) {
                let partner = this.partner_by_id[card.partner_id[0]]
                if (partner) {
                    if (partner['name']) {
                        str += '|' + partner['name'];
                    }
            ...
            str = '' + card['id'] + ':' + str.replace(':', '') + '\n';
            return str;
        },

        search_cards: function (query) {
            query = query.replace(/[\[\]\(\)\+\*\?\.\-\!\&\^\$\|\~\_\{\}\:\,\\\/]/g, '.');
            query = query.replace(' ', '.+');
            const re = RegExp("([0-9]+):.*?" + query, "gi");
            let results = [];
            for (let i = 0; i < this.limit; i++) {
                let r = re.exec(this.card_search_string);
                if (r && r[1]) {
                    let id = r[1];
                    if (this.card_by_id[id]) {
                        results.push(this.card_by_id[id]);
                    }
                } else {
                    break;
                }
        ...
        get_cards: function (max_count) {
            let cards = [];
            let max = 0;
            for (let card_id in this.card_by_id) {
                let card = this.card_by_id[card_id];
                cards.push(card);
                max += 1;
                if (max_count > 0 && max >= max_count) {
                    break;
        ...
        save_gift_cards(cards) {
            for (let i = 0; i < cards.length; i++) {
                let card = cards[i]
                let expired_date = field_utils.parse.datetime(card.expired_date);
                card.expired_date = field_utils.format.datetime(expired_date);
                let create_date = field_utils.parse.datetime(card.create_date);
                card.create_date = field_utils.format.datetime(create_date);
                this.card_by_id[card.id] = card
                let carString = this._card_search_string(card)
                this.card_search_string_by_id[card.id] += carString
            }
            this.card_search_string = "";
            this.cards_stored = []
            for (let card_id in this.card_search_string_by_id) {
                this.card_search_string += this.card_search_string_by_id[card_id];
                this.cards_stored.push(this.card_by_id[card_id])
            }
        },
        ...
        _order_search_string: function (order) {
            ...
            for (let pos_order_id in this.pos_order_string_by_id) {
                ...
                source.push({
                    value: pos_order_id, label: label
                })
            }
            return source;
        ...
        saveRequestPrintFromWaiters(requests) {
            let request_printers = this.load('request_printers', []);
            for (let i = 0; i < requests.length; i++) {
                let request = requests[i]
                request_printers = request_printers.filter(r => r['ticket_number'] != request['ticket_number'])
            }
            request_printers = request_printers.concat(requests)
            this.save('request_printers', request_printers);
        },

        removeRequestPrintFromTicketNumber(ticket_number) {
            let request_printers = this.load('request_printers', []);
            request_printers = request_printers.filter(r => r['ticket_number'] != ticket_number)
            this.save('request_printers', request_printers);
        },
