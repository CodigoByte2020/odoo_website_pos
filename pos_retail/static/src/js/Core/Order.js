    ...
    models.Order = models.Order.extend({
        initialize: function (attributes, options) {
            ...
            if (!options.json) {
                ...
                this.checkin_time = field_utils.format.datetime(moment(this.checkin_time), {}, {timezone: false})
                this.served_by = this.pos.user
                this.served_by_id = this.pos.user.id
            }
            ...
            this.on('automatic-wallet-payment', () => {
                const self = this
            ...
            this.on('automatic-credit-payment', async () => {
                const self = this
        ...
        init_from_JSON: function (json) {
            ...
            if (json.served_by_id) {
                this.served_by_id = json.served_by_id
                if (this.pos.user_by_id[this.served_by_id]) {
                    this.served_by = this.pos.user_by_id[this.served_by_id]
                }
            }
            if (json.pre_print) {
                this.pre_print = json.pre_print
            } else {
                this.pre_print = false
            }
            return res;
        },
        ...
        export_as_JSON: function () {
            ...
            if (this.served_by_id) {
                json.served_by_id = this.served_by_id
            }
            if (this.pre_print) {
                json.pre_print = this.pre_print
            } else {
                json.pre_print = false
            }
            return json
        },
        ...
        export_for_printing: function () {
            ...
            if (this.pos.tables && this.pos.tables.length) {
                ...
                receipt['checkout_time'] = field_utils.format.datetime(moment(receipt['checkout_time']), {}, {timezone: false});
                if (order.table) {
            ...
            if (order.served_by) {
                receipt['served_by'] = order.served_by
            }
            receipt['total_discount'] = order.get_total_discount()
            // TODO 1: restore back cashier , who created order before, not current cashier (ex: User A created order and Assign order to User B)
            // TODO 2: And we want restore user A not B to receipt
            if (order.created_user_id && this.pos.user_by_id[order.created_user_id]) {
                receipt['cashier'] = this.pos.user_by_id[order.created_user_id]['name']
            }
            return receipt
        },
        ...
        generate_unique_id: function () {
            ...
            let d = new Date()
            const dt_string = d.toLocaleTimeString()
            uid_order = uid_order + '-' + dt_string
            return uid_order
        },
        ...
        get_screen_data: function () {
            const screen = _super_Order.get_screen_data.apply(this, arguments)
        ...
        async _setOrdertoCancel(remove_reason) {
        ...
        async validateRequestApproveOrNot(line_uid) {
            const requestStatus = await rpc.query({
                model: 'pos.approve.action', method: 'check_from_ui', args: [[], line_uid], context: {}
            })
        ...
        async ask_guest() {
            let {confirmed, payload: number} = await Gui.showPopup('NumberPopup', {
                'title': _t('How many guests in this Table ?'), 'startingValue': 0,
            });
            if (confirmed) {
                ...
                if (value < 1) {
                    ...
                    this.pos.alert_message({
                        title: _t('Alert'), body: _t('Please input guest, and bigger than 1')
                    })
                } else {
        ...
        set_tip: async function (tip) {
            ...
            if (!tip_product) {
                let result = await this.pos.chrome.rpc({
                    model: 'product.product', method: 'force_write', args: [[this.pos.config.tip_product_id[0]], {
                ...
                if (result) {
                    ...
                    return this.pos.alert_message({
                        title: _t('Error'), body: _t('Please check your internet or your Odoo Server Offline Mode')
                    })
                }
        ...
        async setBundlePackItems() {
            ...
            if (selectedLine) {
                ...
                if (combo_items.length == 0) {
                    return this.pos.alert_message({
                        title: _t('Error'), body: selectedLine.product.display_name + _t(' have not set Combo Items')
                    })
                } else {
                    ...
                    let {confirmed, payload: result} = await Gui.showPopup('PopUpSelectionBox', {
                        title: _t('Select Bundle/Pack Items'), items: combo_items
                    })
                    if (confirmed) {
                ...
                return this.pos.alert_message({
                    title: _t('Error'), body: _t('Have not any selected line in cart, Please selected 1 Line')
                })
            }
        },
        async suggestItems() {
            ...
            if (!selectedLine) {
                return this.pos.alert_message({
                    title: _t('Error'), body: _t('Have not any line set selected')
                })
            }
            ...
            if (!crossItems || crossItems.length == 0) {
                return this.pos.alert_message({
                    title: _t('Error'), body: product.display_name + _t(' not active feature Cross Selling')
                })
            }
            let {confirmed, payload: results} = await Gui.showPopup('PopUpSelectionBox', {
                title: _t('Suggest Customer buy more Products with ' + product.display_name), items: crossItems,
            })
            if (confirmed) {
        ...
        async setProductPackaging() {
            ...
            if (!selectedOrder.get_selected_orderline()) {
                return this.pos.alert_message({
                    title: _t('Error'), body: _t('This feature only active with Products has setup Cross Selling')
                })
            }
            ...
            let {confirmed, payload: packSelected} = await Gui.showPopup('SelectionPopup', {
                title: _t('Select sale from Packaging'), list: packList
            })
            if (confirmed) {
                let selectedLine = selectedOrder.get_selected_orderline();
        ...
        async setMultiVariant() {
            ...
            if (!selectedLine) {
                return this.pos.alert_message({
                    title: _t('Error'), body: _t('Your order is blank cart')
                })
            }
            let product = selectedLine.product;
            ...
            if (!variants) {
                return this.pos.alert_message({
                    title: _t('Error'), body: product.display_name + _t(' have not Active Multi Variant')
                })
            }
        ...
        async submitOrderToBackEnd() {
            ...
            const selectionList = this.pos.payment_methods.map((p) => ({
                id: p.id, item: p, name: p.name
            }))
            let {confirmed, payload: selectedItems} = await Gui.showPopup('PopUpSelectionBox', {
        ...
        client_use_voucher: function (voucher) {
            ...
            if (method) {
                ...
                if (voucher_amount > 0) {
                    ...
                    this.pos.alert_message({
                        title: _t('Warning'), body: _t('Selected Order Paid Full, Could not adding more Voucher Value'),
                    });
                }
            } else {
                this.pos.alert_message({
        ...
        set_pricelist: function (pricelist) {
            ...
            if ((lastPricelist && lastPricelist.id != pricelist.id) || (!lastPricelist)) {
                res = _super_Order.set_pricelist.apply(this, arguments)
            }
            ...
            if (!this.is_return && pricelist && pricelist.currency_id && lastPricelist && pricelist['id'] != lastPricelist['id']) {
                ...
                if (lastPricelist && lastPricelist.currency_id && pricelist.currency_id && lastPricelist.currency_id[0] != pricelist.currency_id[0]) {
                    ...
                    let linesToReComputeManualSet = this.get_orderlines().filter((l) => l.price_manually_set)
                    ...
                    // TODO: allow all item set price manual can covert difference currency
                    linesToReComputeManualSet.forEach(function (l) {
                        // 1st: price 10 USD => n0
                        // 2st: price  x EUR => 10 * rate => n1
                        // 3rd: price  x VND => n1 / last rate * vnd rate => n2
                        // 4nd: price  x USD => n2 / last rate * usd rate => n0
                        let price = l.price // n1
                        let lastCurr = self.pos.currency_by_id[lastPricelist.currency_id[0]]
                        let lastRate = lastCurr['converted_currency']
                        let selectedCurr = self.pos.currency_by_id[pricelist.currency_id[0]]
                        if (selectedCurr && lastRate > 0) {
                            let selectedRate = selectedCurr['converted_currency']
                            let nextPrice = price / lastRate * selectedRate
                            l.set_unit_price(nextPrice);
                            self.fix_tax_included_price(l);
                        }
                    })
                }
            ...
            if (!this.element) {
                try {
                    JsBarcode('#' + element, this['ean13'], {
                        format: "EAN13", displayValue: true, fontSize: 14
                    });
        ...
        async alertOrdersOfClientNotPaidFull(client) {
            ...
            if (partial_payment_orders.length != 0) {
                ...
                if (due_amount != 0) {
                    ...
                    if (confirmed) {
                        const {confirmed, payload: result} = await Gui.showTempScreen('PosOrderScreen', {
                            order: partial_payment_orders[0], selectedClient: client
                        });
                    }
                }
            } else {
        ...
        set_client: async function (client) {
            ...
            if (client && !this.pos.the_first_load) {
                if (this.pos.config.pos_orders_management && this.pos.config.pos_order_warning_customer_orders_pending_paid) {
                    setTimeout(() => {
                ...
                if (client.group_ids.length > 0) {
                    ...
                    if (lists.length > 0) {
                        const {confirmed, payload: group} = await Gui.showPopup('SelectionPopup', {
                            title: this.pos.env._t('Choice one Group/MemberShip'), list: lists
                        })
                        if (confirmed) {
            ...
            if (client) {
                ...
                if (!pricelistOfClient || (currentPricelist && pricelistOfClient && currentPricelist.id != pricelistOfClient.id)) {
            ...
            if (client) {
                this.pos.alert_message({
                    title: _t('Successfully'), body: client['name'] + _t(' Set to order !')
                })
            } else {
                this.pos.alert_message({
                    title: _t('Successfully'), body: _t('Deselected Customer !')
                })
            }
            if (client && this.pos.config.notification_last_days_purchase && !this.pos.the_first_load) {
                this.pos.rpc({
                    model: 'pos.config', method: 'checkCustomerLastPurchase', args: [[], client.id], context: {}
                }, {
                    shadow: true, timeout: 6500,
                }).then(function (days) {
                    if (days > 0) {
                        if (days == 1) {
                            self.pos.alert_message({
                                title: client.name, body: _t('just purchase today')
                            })
                        } else {
                            self.pos.alert_message({
                                title: client.name, body: days + _t(' days not purchase')
                            })
                        }
                    } else {
                        self.pos.alert_message({
                            title: client.name, body: _t('First time purchase')
                        })
                    }
                }, function (err) {
                    return self.pos.query_backend_fail(err)
        ...
        async add_shipping_cost(service, product, is_shipping_cost) {
            if (service['type'] == 'fixed') {
                await this.add_product(product, {
                    price: service.amount, quantity: 1, merge: false, extras: {
                ...
                if (amount_total > 0) {
                    ...
                    await this.add_product(product, {
                        price: price, quantity: 1, merge: false, extras: {
                            service_id: service.id,
                        }
                    });
        ...
        validate_global_discount: function () {
            ...
            if (client && client['discount_id']) {
                ...
                return Gui.showPopup('ConfirmPopup', {
                    'title': _t('Customer special discount ?'), 'body': body, confirm: function () {
        ...
        validate_payment_order: function () {
            ...
            if (this && this.orderlines.models.length == 0) {
                this.pos.gui.show_screen('products');
                return this.pos.alert_message({
                    title: _t('Warning'), body: _t('Your order is blank cart'),
                })
            } else {
                if (this.get_total_with_tax() == 0) {
                    this.pos.alert_message({
                        title: _t('Warning'), body: _t('Your order have total paid is 0, please take careful')
                    })
                }
            }
            if (this.remaining_point && this.remaining_point < 0) {
                this.pos.gui.show_screen('products');
                return this.pos.alert_message({
                    title: _t('Warning'), body: _t('You could not applied redeem point bigger than client point'),
                });
            }
            ...
            if (this.is_to_invoice() && !this.get_client()) {
                ...
                this.pos.alert_message({
                    title: _t('Warning'), body: _t('Please add client the first')
                });
                return false;
            }
            return true
        ...
        validate_order_return: function () {
            if (this.pos.config.required_reason_return) {
                ...
                if (line_missed_input_return_reason) {
                    this.pos.gui.show_screen('products');
                    return this.pos.alert_message({
                        title: _t('Alert'), body: _t('Please input return reason for each line'),
                    });
                } else {
                    return false
                }
        ...
        async set_discount_value(discount) {
            ...
            if (discount < 0) {
                return Gui.showPopup('ErrorPopup', {
                    title: _t('Error'), body: _t('It not possible set Discount Value smaller than 0')
                })
            }
        ...
        add_order_credit: function () {
            ...
            if (this.add_credit && !this.get_client()) {
                this.pos.gui.show_screen('clientlist');
                return this.pos.alert_message({
                    title: _t('Warning'), body: 'Please add customer need add credit'
                })
            }
        },
                ...
                return Gui.showPopup('popup_selection_extend', {
                    ...
                    confirm: function (generic_option_ids) {
                        if (generic_option_ids.length == 0) {
                            ...
                            return this.pos.alert_message({
                                title: _t('Warning'), body: _t('Required select one Generic Option')
                            })
                        } else {
            ...
            if (this.pos.config.mrp && selected_orderline && selected_orderline.is_has_bom()) {
                ...
                if (boms.length = 1) {
                    ...
                    for (let i = 0; i < bom_line_ids.length; i++) {
                        bom_lines.push({
                            id: bom_line_ids[i].id, quantity: bom_line_ids[i].product_qty,
                        })
                    }
                    if (bom_lines.length) {
        ...
        set_tags: function () {
            if (this && this.selected_orderline) {
                ...
                return Gui.showPopup('popup_selection_tags', {
                    selected_orderline: selected_orderline, title: this.pos.env._t('Add Tags')
                });
            } else {
                return this.pos.alert_message({
                    title: this.pos.env._t('Warning'), body: this.pos.env._t('Your shopping cart is empty'),
                })
            }
        },
        set_seller: function () {
            ...
            if (confirmed) {
                ...
                if (!error) {
                    let voucher = await rpc.query({
                        model: 'pos.voucher', method: 'create_from_ui', args: [[], values], context: {}
                    })
        ...
        async setRewardProgram(reward) {
            ...
            if (!product) {
                let resultUpdate = await this.pos.rpc({
                    model: 'product.product', method: 'force_write', args: [[loyalty.product_loyalty_id[0]], {
                        'available_in_pos': true, 'sale_ok': true, 'active': true,
                    }], context: {}
                })
                if (resultUpdate) {
            ...
            if (client['pos_loyalty_point'] <= redeem_point_used) {
                return Gui.showPopup('ErrorPopup', {
                    title: _t('Warning'), body: _t('Point of customer not enough'),
                })
            }
            ...
            if (reward['type'] == 'discount_products') {
                ...
                if ((client['pos_loyalty_point'] > (point_will_redeem + redeem_point_used)) && price_discount) {
                    applied = true;
                    await this.add_product(product, {
                        price: price_discount, quantity: -1, merge: false, extras: {
                            reward_id: reward.id, redeem_point: point_will_redeem
                        }
                    });
                    return Gui.showPopup('ConfirmPopup', {
                        title: _t('Successfully'), body: _t('Set Discount: ') + this.pos.format_currency(price_discount)
                    })
                }
                    ...
                    if (product_gift) {
                        ...
                        if (client['pos_loyalty_point'] > (point_will_redeem + redeem_point_used)) {
                            ...
                            await this.add_product(product_gift, {
                                price: 0, quantity: reward['quantity'], merge: false, extras: {
                                    reward_id: reward.id, redeem_point: point_will_redeem
                                }
                            });
                            return Gui.showPopup('ConfirmPopup', {
                                title: _t('Successfully'), body: _t('Set Gift: ') + product_gift.display_name
                            })
                        }
                    }
                }
                ...
                if (client['pos_loyalty_point'] > (point_will_redeem + redeem_point_used)) {
                    applied = true;
                    await this.add_product(product, {
                        price: amount_total, quantity: -1, merge: false, extras: {
                            reward_id: reward.id, redeem_point: point_will_redeem
                        }
                    });
                    return Gui.showPopup('ConfirmPopup', {
                        title: _t('Successfully'), body: _t('Set Discount: ') + this.pos.format_currency(amount_total)
                    })
                }
                ...
                let {confirmed, payload: point} = await Gui.showPopup('NumberPopup', {
                    title: title, startingValue: pointCanUse
                })
                if (confirmed) {
                    ...
                    if (client['pos_loyalty_point'] < next_redeem_point) {
                        ...
                        if (next_amount >= 0) {
                            applied = true;
                            await this.add_product(product, {
                                price: -(point * reward['coefficient']), quantity: 1, merge: false, extras: {
                                    reward_id: reward.id, redeem_point: point
        ...
        lock_order: async function () {
            ...
            if (order && order.table) {
                let result = await rpc.query({
                    model: 'restaurant.table', method: 'lock_table', args: [[order.table.id], {
                        'locked': true,
                    }],
                })
                if (result) {
        ...
        create_sale_order: function () {
            ...
            if (order.get_total_with_tax() <= 0) {
                return this.pos.alert_message({
                    title: _t('Warning'), body: _t("Amount total of order required bigger than 0"),
                });
            }
        ...
        get_promotions_active: function () {
            if (this.is_return) {
                return {
                    can_apply: false, promotions_active: []
                };
            }
            let can_apply = null;
            ...
            if (!this.pos.promotions) {
                return {
                    can_apply: can_apply, promotions_active: []
                };
            }
            ...
            return {
                can_apply: can_apply, promotions_active: promotions_active
            };
        },
        apply_promotion: function (promotions) {
            ...
            if (this.is_return) {
                return Gui.showPopup('ConfirmPopup', {
                    title: _t('Warning'), body: _t('Return order not allow apply promotions'),
                });
            }
            if (!promotions) {
            ...
            while (i < gifts.length) {
                ...
                if (product) {
                    ...
                    this.add_promotion_gift(product, 0, qty_free, {
                        promotion: true, promotion_gift: true, promotion_reason: promotion.name
                    })
                } else {
                    this.pos.alert_message({
                        title: _t('Warning'),
            ...
            if (rules_apply && product_discount) {
                for (let n = 0; n < rules_apply.length; n++) {
                    ...
                    this.add_promotion_gift(product_discount, promotion_amount, -1, {
                        promotion: true, promotion_reason: promotion_reason
                    })
                }
            }
        },
        ...
        compute_buy_x_get_another_free: function (promotion) {
            ...
            for (let index_id in promotion.product_ids) {
                ...
                if (total_quantity[product_id] && total_quantity[product_id] >= minimum_items) {
                    ...
                    if (!product) {
                        return this.pos.alert_message({
                            title: _t('Error'), body: 'Product id ' + product_id + ' not available in pos'
                        })
                    }
                    this.add_promotion_gift(product, 0, -qty_free, {
                        promotion: true, promotion_reason: promotion.name
                    })
                }
            }
        },
    ...
    models.Orderline = models.Orderline.extend({
            ...
            if (this.combo_items && this.combo_items.length) {
                ...
                for (let n = 0; n < this.combo_items.length; n++) {
                    json.combo_item_ids.push({
                        id: this.combo_items[n].id, quantity: this.combo_items[n].quantity
                    })
                }
            }
            if (this.uom_id) {
            ...
            if (this.lot_ids) {
                ...
                for (let i = 0; i < this.lot_ids.length; i++) {
                    ...
                    pack_lot_ids.push([0, 0, {
                        lot_name: lot['name'], quantity: lot['quantity'], lot_id: lot['id']
                    }]);
                }
        ...
        set_unit_price: async function (price) {
            ...
            if (!this.is_return && this.pos.config.validate_minimum_price) {
                ...
                if (minimumPrice) {
                    this.pos.alert_message({
                        title: _t('Warning'), body: minimumPrice
                    });
                }
            }
        ...
        set_unit: function (uom_id, price_extra) {
            if (!this.pos.the_first_load && !uom_id) {
                return false
            }
            this.uom_id = uom_id
            if (!price_extra || price_extra == 0) {
                ...
                this.set_unit_price(price_extra)
            }
            return true
        },
            ...
            if (this.product.uom_ids && this.product.uom_ids.length) {
                ...
                for (let i = 0; i < this.product.uom_ids.length; i++) {
                    ...
                    for (let n = 0; n < pricelist.items.length; n++) {
                        ...
                        if ((!item.product_tmpl_id || item.product_tmpl_id[0] === this.product.product_tmpl_id) && (!item.product_id || item.product_id[0] === this.product.id) && (!item.categ_id || _.contains(category_ids, item.categ_id[0])) && (!item.date_start || moment(item.date_start).isSameOrBefore(date)) && (!item.date_end || moment(item.date_end).isSameOrAfter(date))) {
            ...
            if (!this.bom_lines) {
                ...
                for (let i = 0; i < this.bom_lines.length; i++) {
                    ...
                    bom_lines_added.push({
                        bom_line: bom_line, quantity: bom_line_item.quantity
                    })
                }
        ...
        is_has_bom: function () {
            ...
            if (this.pos.bom_by_product_id && this.pos.bom_by_product_id[this.product.id]) {
                ...
                const boms = this.pos.boms.filter(b => b.product_tmpl_id[0] == this.product.product_tmpl_id)
                if (boms.length > 0) {
                    return boms
                }
            }
        ...
        async _openPrice() {
            let {confirmed, payload: number} = await Gui.showPopup('NumberPopup', {
                'title': _t('What Price of Item ?'), 'startingValue': 0,
            });
            if (confirmed) {
