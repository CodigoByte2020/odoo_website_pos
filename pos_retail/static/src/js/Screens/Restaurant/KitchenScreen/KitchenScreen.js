odoo.define('pos_retail.KitchenScreen', function (require) {
    ...
    class KitchenScreen extends IndependentToOrderScreen {
        constructor() {
            ...
            this.state = {
                query: null, selectedOrder: this.props.selectedOrder, orders: this.orders || []
            };
        ...
        get orderList() {
            ...
            // -----------------------------------
            // TODO: if screen type is kitchen, loop each line check allow display filter by product_categ_ids, if product's category inside product_categ_ids allow display
            // -----------------------------------
            if (this.env.pos.config.screen_type == 'kitchen') {
        ...
        newOrderReceiptsComing(receiptOrder) {
            console.log('>>> newOrderReceiptsComing')
        ...
        orderReceiptRemoved(vals) {
            this.env.pos.alert_message({
                title: this.env._t('Alert'), body: this.env._t('have 1 Ticket Removed: ' + vals.uid)
            })
        ...
        async transferItems(event) { // method for waiters/cashiers only
            ...
            if (linesWaitingKitchen.length == 0) {
                ...
                posbus.trigger('syncTransferSucceedReceipt', {
                    request_time: orderReceipt.request_time, action: 'Done', user: this.env.pos.user.name
                })
        ...
        doneLine(event) { // method for kitchen only, them done each line receipt
            ...
            const reportHtml = QWeb.render('KitchenRequestItem', {
                pos: this.env.pos, change: line, order: order
            });
            if (this.env.pos.proxy.printer) {
                this.env.pos.proxy.printer.print_receipt(reportHtml)
            } else {
                this.showScreen('ReportScreen', {
                    report_html: reportHtml, report_xml: null, orderRequest: order
                })
            }
        ...
        async cancelLine(event) {
            ...
            if (this.env.pos.config.required_input_reason_cancel) {
                let {confirmed, payload: result} = await this.showPopup('PopUpSelectionBox', {
                    title: this.env._t('Why cancel this transaction ?'), items: this.env.pos.cancel_reasons
                })
                if (confirmed) {
        ...
        async forceDoneAllReceipts() {
            if (this.state.orders.length > 0) {
                ...
                this.env.pos.alert_message({
                    title: this.env._t('Error'), body: this.env._t('Your Kitchen Screen is blank')
                })
            }
        }
        ...
        async printOrder(orderRequest) {
            ...
            if (!printers || printers.length == 0) {
                return this.env.pos.alert_message({
                    title: this.env._t('Error'), body: this.env._t('You pos not set Order Printers')
                })
            }
            ...
            if (!printerNetwork && !printerViaPOSBOX) { // todo: if pos not set proxy ip or printer network we return back odoo original
                this.env.pos.alert_message({
                    title: this.env._t('Warning'), body: this.env._t('Your POS Config not setup POSBOX or IOT Boxes')
                })
            } else {
        ...
        get filterOptions() {
            ...
            this.sepecialFilter.set('all', {
                key: 'all', text: this.env._t('All Tickets'),
            })
            this.sepecialFilter.set('priority', {
                key: 'priority', text: this.env._t('Priority'),
            })
            this.sepecialFilter.set('low_priority', {
                key: 'low_priority', text: this.env._t('Low Priority'),
            })
            if (!this.env.pos.tables) {
                return this.sepecialFilter
            } else {
                this.env.pos.tables.forEach(t => this.sepecialFilter.set(t.name, {
                    key: t.name, text: t.name,
                }))
                return this.sepecialFilter
            }
