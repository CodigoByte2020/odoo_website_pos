odoo.define('pos_retail.restaurant', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        // TODO: if pos user click to table, auto save order with state draft in backend

        // Todo: when pos user click to back to floor, auto save last selected Order json to backend
        sync_from_server: function (table, table_orders, order_ids) { // save draft orders to server
            return _super_posmodel.sync_from_server.apply(this, arguments);
        },

        // TODO: if pos user click to table, auto save order with state draft in backend
        sync_to_server: function (table, order) {
            return _super_posmodel.sync_to_server.apply(this, arguments);
        },
        ...
        _get_from_server: function (table_id, options) {
            ...
            return this.rpc({
                ...
                args: [this.pos_session.id, table_id],
                kwargs: {context: this.session.user_context},
            }, {
                timeout: timeout, shadow: false,
            })
        },
    ...
    models.Orderline = models.Orderline.extend({
        initialize: function (attributes, options) {
            ...
            if (!options.json && this.pos.config.sync_multi_session && this.pos.config.screen_type != 'kitchen') {
                this.kitchen_notes = '';
                this.cancel_reason = ''
