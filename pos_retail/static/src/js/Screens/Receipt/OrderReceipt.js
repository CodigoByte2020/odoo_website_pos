odoo.define('pos_retail.OrderReceipt', function (require) {
    ...
    const RetailOrderReceipt = (OrderReceipt) => class extends OrderReceipt {
        constructor() {
            super(...arguments);
            this.state = useState({
        ...
        get _getStyleOfReceipt() {
            if (this.env.pos.config.receipt_fix && this.env.pos.config.receipt_fix_width > 0) {
                let styleReceipt = "width: " + this.env.pos.config.receipt_fix_width + "mm; "
                styleReceipt += "margin: " + this.env.pos.config.receipt_fix_margin + "mm; "
                styleReceipt += "margin-left: " + this.env.pos.config.receipt_fix_margin_left + "mm; "
                styleReceipt += "margin-right: " + this.env.pos.config.receipt_fix_margin_right + "mm; "
                styleReceipt += "font-size: " + this.env.pos.config.receipt_fix_font_size + "px; "
                return styleReceipt
            } else {
                return ""
            }
                        ...
                        if ((currentOrder.syncing == false || !currentOrder.syncing) && this.env.pos.pos_bus && !this.env.pos.splitbill) {
                            this.env.pos.pos_bus.requests_printers.push({
                                action: 'request_printer', data: {
                                    uid: currentOrder.uid, computeChanges: orderReceipt,
