odoo.define('pos_retail.TicketScreen', function (require) {
    ...
    const RetailTicketScreen = (TicketScreen) => class extends TicketScreen {
        constructor() {
            super(...arguments);
            NumberBuffer.use({
        ...
        async removeAllOrders() {
            let {confirmed, payload: result} = await this.showPopup('ConfirmPopup', {
                title: this.env._t('Warning'), body: this.env._t('Are you sure remove all Orders ?')
            })
            if (confirmed) {
        ...
        async _onDeleteOrder({detail: order}) {
            if (!this.env.pos.config.allow_remove_order && !order['temporary']) {
            ...
            let {confirmed, payload: note} = await this.showPopup('TextAreaPopup', {
                ...
                body: `${order.name} has total amount of ${this.env.pos.format_currency(order.get_total_with_tax())}, are you sure you want delete this order?`,
            });
            if (!confirmed) {
                return
            } else {
                ...
                await order._setOrdertoCancel(note)
        ...
        async saveToPartialOrder(selectedOrder) {
            let {confirmed, payload: confirm} = await this.showPopup('ConfirmPopup', {
                title: this.env._t('Alert'), body: this.env._t("Are you want save current Order to Draft Order ?"),
            })
            if (confirmed) {
                ...
                let lists = this.env.pos.payment_methods.filter((p) => (p.journal && p.pos_method_type && p.pos_method_type == 'default') || (!p.journal && !p.pos_method_type)).map((p) => ({
                    id: p.id, item: p, label: p.name
                }))
                let {confirmed, payload: paymentMethod} = await this.showPopup('SelectionPopup', {
                    title: this.env._t('Save Order to Partial Order, Please select one Payment Method !!'), list: lists
                })
                if (confirmed) {
