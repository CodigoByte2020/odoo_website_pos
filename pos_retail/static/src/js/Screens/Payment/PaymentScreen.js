odoo.define('pos_retail.PaymentScreen', function (require) {
    ...
    const RetailPaymentScreen = (PaymentScreen) => class extends PaymentScreen {
        constructor() {
            super(...arguments);
        ...
        get currentOrder() {
            let currentOrder = super.currentOrder
            if (!currentOrder && this._lastOrder) {
        ...
        _updateCurrentOrder(pos, newSelectedOrder) {
            if (newSelectedOrder) {
                this._lastOrder = this._currentOrder
        ...
        async _scanVoucherCode(code) {
            ...
            const voucher = await this.env.pos.rpc({
                model: 'pos.voucher', method: 'get_voucher_by_code', args: [code],
            })
            if (voucher == -1) {
                return this.showPopup('ErrorPopup', {
                    title: this.env._t('Error'), body: this.env._t('Not found any Voucher or Voucher Expired Date')
                });
            } else {
        ...
        async validateOrder(isForceValidate) {
            ...
            if (this._currentOrder && !this._currentOrder.is_paid()) {
                return this.showPopup('ErrorPopup', {
                    title: this.env._t('Warning'), body: this.env._t('Please full fill payment Amount'),
                })
            }
            ...
            // todo: we need fast than for cashier get a bill, for order submit to backend, can work in background
        ...
        async changeCurrency() {
            ...
            const list = this.env.pos.multi_currencies.map(c => ({
                id: c.id, label: c.name, isSelected: false, item: c
            }))
            let {confirmed, payload: currency} = await this.showPopup('SelectionPopup', {
                title: this.env._t('Choice Currency for Payment Order'), list: list,
            });
            if (confirmed) {
        ...
        async askAddChargeAmount(method) {
            ...
            if (dueAmount <= 0) {
                return this.showPopup('ErrorPopup', {
                    title: this.env._t('Warning'), body: this.env._t('Order Full Fill Payments Amount')
                })
            }
                ...
                if (productFee) {
                    ...
                    if (feesAmount != 0) {
                        ...
                        return await this.currentOrder.add_product(productFee, {
                            quantity: 1, price: feesAmount, merge: false
                        });
                    } else {
                        return this.showPopup('ErrorPopup', {
                            title: this.env._t('Error'), body: this.env._t('Fees Amount it not Possible is 0')
                        })
                    }
                } else {
                    return this.showPopup('ErrorPopup', {
                ...
                if (productDisc) {
                    ...
                    if (discountAmount != 0) {
                        ...
                        return await this.currentOrder.add_product(productDisc, {
                            quantity: 1, price: discountAmount, merge: false
                        });
                    } else {
                        return this.showPopup('ErrorPopup', {
                            title: this.env._t('Error'), body: this.env._t('Fees Amount it not possible is 0')
        ...
        setCoin(event) {
            ...
            if ((!selectedPaymentline) || (selectedPaymentline.payment_method && selectedPaymentline.payment_method.pos_method_type != 'default')) {
                ...
                if (!cashMethod) {
                    this.env.pos.alert_message({
                        title: this.env._t('Error'), body: this.env._t('Cash method not found in your pos !'),
                    });
                } else {
                    this.currentOrder.add_paymentline(cashMethod);
        ...
        async setReferencePayment(event) {
            ...
            let {confirmed, payload: ref} = await this.showPopup('TextInputPopup', {
                title: this.env._t('Payment Reference Notes ?'), startingValue: line.ref || ''
            })
            if (confirmed) {
        ...
        async _isOrderValid() {
            ...
            if (this.currentOrder) {
                ...
                if (!this.env.pos.config.allow_payment_zero && totalWithTax == 0) {
                    this.env.pos.alert_message({
                        title: this.env._t('Error'),
                        body: this.env._t('Your POS not allow payment order with Amount Total is 0, required difference 0'),
                    });
        ...
        async inputVoucherCode() {
            ...
            if (due <= 0) {
                return this.showPopup('ErrorPopup', {
                    title: this.env._t('Error'), body: this.env._t('Order full fill Amount, can not use Voucher')
                })
            }
            ...
            if (confirmed) {
                ...
                if (code) {
                    ...
                    return this.showPopup('ErrorPopup', {
                        title: this.env._t('Warning'), body: this.env._t('Voucher Code not found'),
                    })
                }
            }
        }
            ...
            if (confirmed) {
                ...
                if (confirmed) {
                    ...
                    if (!error) {
                        ...
                        this.env.pos.alert_message({
                            title: this.env._t('Error'), body: error,
                        })
                    }
                }
            }
        ...
        async selectLoyaltyReward() {
            let client = this.currentOrder.get_client();
            if (!client) {
                const {confirmed, payload: newClient} = await this.env.pos.chrome.showTempScreen('ClientListScreen', {
            ...
            if (confirmed) {
                const list = this.env.pos.rewards.map(reward => ({
                    id: reward.id, label: reward.name, isSelected: false, item: reward
                }))
                let {confirmed, payload: reward} = await this.showPopup('SelectionPopup', {
                    title: this.env._t('Please select one Reward need apply to customer'), list: list,
                });
                if (confirmed) {
        ...
        async saveToWallet() {
            ...
            if (due >= 0) {
                return this.showPopup('ErrorPopup', {
                    title: this.env._t('Error'), body: this.env._t('Order have not change amount for save to Wallet')
                })
            }
            let self = this;
            ...
            let {confirmed, payload: number} = await this.showPopup('NumberPopup', {
                title: this.env._t('Which wallet amount save to Wallet of Customer ?'), startingValue: changeAmount
            })
            if (confirmed) {
        ...
        async useCreditPaid() {
            ...
            if (amountDue <= 0) {
                return this.showPopup('ErrorPopup', {
                    title: this.env._t('Error'), body: this.env._t('Due amount required bigger than 0')
                })
            }
            if (!this.currentOrder.get_client()) {
                const {confirmed, payload: newClient} = await this.showTempScreen('ClientListScreen', {
