odoo.define('pos_retail.Orderline', function (require) {
    ...
    const RetailOrderline = (Orderline) => class extends Orderline {
        constructor() {
            super(...arguments);
        }
        ...
        mounted() {
            super.mounted()
            this.props.line.on('change', this._reloadLine, this)
            console.log("mounted to line: " + this.props.line.uid)
        }

        _reloadLine() {
            this.render()
            console.log('>>> _reloadLine() line have change. uid: ' + this.props.line.uid)
        }

        willUnmount() {
            super.willUnmount()
            this.props.line.off('change', null, this)
            console.log("willUnmount to line: " + this.props.line.uid)
        }
        ...
        async setTags() {
            ...
            if (!selectedLine) {
                return this.env.pos.alert_message({
                    title: this.env._t('Error'), body: this.env._t('Have not Selected Line')
                })
            }
            ...
            let {confirmed, payload: results} = await this.showPopup('PopUpSelectionBox', {
                title: this.env._t('Select Tags/Notes'), items: tags
            })
            if (confirmed) {
        ...
        async setNotes() {
            ...
            const {confirmed, payload: inputNote} = await this.showPopup('TextAreaPopup', {
                startingValue: selectedLine.get_note(), title: this.env._t('Add Note'),
            });
        ...
        async setAddons() {
            ...
            if (addons.length) {
                let {confirmed, payload: result} = await this.showPopup('PopUpSelectionBox', {
                    title: this.env._t('Select Add-ons Items'), items: addons
                })
                if (confirmed) {
                ...
                this.env.pos.alert_message({
                    title: this.env._t('Warning'), body: this.env._t('Products of Addon not available in POS')
                })
            }
        }
        ...
        async modifiersComboItem(combo_item) {
            let lists = [{
                name: this.env._t('Full Size'), item: 1, id: 1,
            }, {
                name: this.env._t('Left and Right Haft'), item: 2, id: 2,
            }, {
                name: this.env._t('4 Quaters'), item: 3, id: 3,
            },]
            let title = this.env._t('Select type of Modifier')
            let {confirmed, payload: selectedCloseTypes} = await this.showPopup('PopUpSelectionBox', {
                title: title, items: lists, onlySelectOne: true,
            });
            let selectedType;
            ...
            if (confirmed && selectedType) {
                ...
                if (selectedType.id == 1) {
                    arrays = [{
                        name: this.env._t('Full Size'), id: 1,
                    },]
                }
                if (selectedType.id == 2) {
                    arrays = [{
                        name: this.env._t('Left Haft'), id: 1,
                    }, {
                        name: this.env._t('Right Haft'), id: 2,
                    }]
                }
                if (selectedType.id == 3) {
                    arrays = [{
                        name: this.env._t('1st. Left Quater'), id: 1,
                    }, {
                        name: this.env._t('2nd. Left Quater'), id: 2,
                    }, {
                        name: this.env._t('3rd. Right Quater'), id: 3,
                    }, {
                        name: this.env._t('4th. Right Quater'), id: 4,
                    }]
                }
                let {confirmed, payload: modifiers} = await this.showPopup('PopUpModifierProductAttributes', {
        ...
        async modifiersProductAttributeValues() {
            ...
            let {confirmed, payload: valuesSelected} = await this.showPopup('PopUpModifiersAttributeValues', {
        ...
        async setUnit() {
            ...
            let {confirmed, payload: unit} = await this.showPopup('SelectionPopup', {
                title: this.env._t('Select Unit of Measure for : ') + this.props.line.product.display_name, list: list
            })
            if (confirmed) {
        ...
        async setSeller() {
            ...
            let {confirmed, payload: seller} = await this.showPopup('SelectionPopup', {
                title: this.env._t('Please select one Seller'), list: list
            })
            if (confirmed) {
        ...
        async showAllLots() {
            ...
            if (confirmed && selectedLot) {
                ...
                const {confirmed, payload} = await this.showPopup('EditListPopup', {
                    title: this.env._t('Lot/Serial Number(s) Required'), isSingleItem: false, array: packLotLinesToEdit,
                });
                if (confirmed) {
        ...
        async produceProduct() {
            ...
            let {confirmed, payload: results} = await this.showPopup('PopUpCreateMrpOrder', {
                title: this.env._t('Modifiers BOM and Create MRP Order'), items: bom_lines_set
            })
            if (confirmed) {
        ...
        async modifiersAttributes() {
            if (this.env.pos.config.product_configurator && _.some(this.props.line.product.attribute_line_ids, (id) => id in this.env.pos.attributes_by_ptal_id)) {
                ...
                let {confirmed, payload} = await this.showPopup('ProductConfiguratorPopup', {
                    product: this.props.line.product, attributes: attributes,
                });
