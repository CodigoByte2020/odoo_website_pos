...
class POSOrder(models.Model):
    ...
    served_by_id = fields.Many2one('res.users', 'Served By')
    ...
    order_json = fields.Binary('Order Json', attachment=False, readonly=1)
    remove_reason = fields.Text("Remove Reason", readonly=1)
    removed_time = fields.Datetime("Removed Time", readonly=1)
    ...
    def write(self, vals):
        ...
        if vals.get('state', None) and vals.get('state', None) == 'cancel':
            vals.update({'removed_time': fields.Datetime.now()})
    ...
    def _order_fields(self, ui_order):
        ...
        if ui_order.get('served_by_id', False):
            order_fields.update({
                'served_by_id': ui_order['served_by_id']
            })
        return order_fields
    ...
    def _process_order(self, order, draft, existing_order):
        ...
        if draft:
            self.browse(order_id).write({
                'order_json': base64.encodebytes(json.dumps(order['data'], default=date_utils.json_default).encode('utf-8')),
            })
            _logger.info('-->>>>>>>> save order with mode draft')
        return order_id
    ...
    def create_from_ui(self, orders, draft=False):
        ...
        _logger.info(
            'create_from_ui() with times:  %s ' % (timeit.default_timer() - start))
