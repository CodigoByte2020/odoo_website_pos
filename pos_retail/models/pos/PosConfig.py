...
class PosConfig(models.Model):
    ...
    product_default_ids = fields.Many2many(
        'pos.product.default.new.cart',
        'pos_config_pos_product_default_new_cart_rel',
        'config_id',
        'default_id',
        string='Product Default add to cart'
    )
    bluetooth_printer = fields.Boolean('Bluetooth Printer', default=0)
    bluetooth_receipt_copies = fields.Integer('Copies of receipts', default=0)
    bluetooth_device_ip_address = fields.Char("Bluetooth Device Ip Address", default='localhost')
    ...
    pos_orders_management = fields.Boolean(
        ...
        default=0)
    pos_order_warning_customer_orders_pending_paid = fields.Boolean(
        'Warning Cashier Customer have Orders not paid',
        default=0,
        help='If cashier set Customer to cart \n'
             'And if customer have orders not yet paid full \n'
             'POS will show popup list out count orders not paid full'
    )
    ...
    receipt_fix_width = fields.Integer('Receipt Width (mm)', default=150)
    ...
    discount_value_dynamic = fields.Boolean(
        'Discount Value Dynamic',
        help='Allow cashier set discount each product, and discount value required smaller than sale price'
    )
    ...
    cache = fields.Selection([
        ('none', 'None'),
        ('browse', 'Stored POS Counter'),
    ],
        default='none',
        string='Loading Products and Customer Cache Type',
        help='1) None: Loading all customers and products direct Odoo Server \n'
             '2) Stored POS Counter: All customers and products will Cached to POS Counter \n'
             '   - First time all customers and products will save to POS Counter Device \n'
             '   - After first time, when pos resume or start new session, customers \n'
             'and products will loading direct POS Counter Device \n'
             '   - Customers and Products datas automatic sync with update of backend'
    )
    cache_sync_automatic = fields.Boolean('Automatic Sync Cache', default=0)
    cache_sync_automatic_time = fields.Integer(
        'Schedule Time Sync Cache (seconds)',
        default=60
    )
