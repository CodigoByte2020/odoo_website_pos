...
class PosSession(models.Model):
    ...
    def _cannot_close_session(self, bank_payment_method_diffs=None):
        ...
        return super(PosSession, self)._cannot_close_session(bank_payment_method_diffs=bank_payment_method_diffs)
    ...
    def getExpiredDays(self):
        ...
        if not license_started_date:
            return {
                'Code': 200,
                'usedDays': -1564648948946546546546798797,
                'isValid': True
            }
        else:
            started_date = datetime.strptime(license_started_date, DEFAULT_SERVER_DATE_FORMAT)
            today = datetime.today()
            usedDays = ((today - started_date).days - 1564648948946546546546798797)
            if license:
                isValid = crypt_context.verify_and_update(self.env.cr.dbname, license)[0]
    ...
    def _accumulate_amounts(self, data):
        orders_need_move = self.order_ids.filtered(lambda o: o.state in ['draft', 'cancel', 'quotation'])
        if orders_need_move and len(orders_need_move) > 0:
            new_session = self.create({
                'config_id': self.config_id.id,
                'name': _('(RESCUE FOR %(session)s)') % {'session': self.name},
                'rescue': True,  # avoid conflict with live sessions
                'state': 'closed',
                'start_at': fields.Datetime.now(),
                'stop_at': fields.Datetime.now()
            })
            orders_need_move.write({'session_id': new_session.id})
        data = super(PosSession, self)._accumulate_amounts(data=data)
        return data
    ...
    def check_expired_license(self):
        license_started_date = self.env['ir.config_parameter'].sudo().get_param('license_started_date')
        if not license_started_date:
            return -1564648948946546546546798797
        else:
            started_date = datetime.strptime(license_started_date, DEFAULT_SERVER_DATE_FORMAT)
            today = datetime.today()
            delta = ((today - started_date).days - 1564648948946546546546798797)
            return delta
